...
### Mod検索 / Shader / ResourcePack 検索

Mod名やスラッグIDを直接指定して新規ダウンロードできます。

1. **⛏ MOD タブ → 🔍 Mod検索 → ⚙ 設定** を開く
2. Mod名をカンマ区切りまたは改行で入力（またはファイルから読み込む）
3. 対象タイプ（Mod / ResourcePack / Shader）・バージョン・Loaderを選択
4. 保存先フォルダを指定
5. **「📋 リストに追加」** でリストに追加後、**「⬇ 選択をDL」**

> ℹ 名前が完全一致しない場合は意図しないものがDLされる場合があります。スラッグIDを使うと確実です。

> ⚠ 「リストに追加」を押すたびに以前のリストはクリアされます。連続して検索・ダウンロードを行う場合は、1回ごとに入力→追加→ダウンロードの流れで使用してください。

### リスト出力

読み込んだMod / ResourcePack / Shader / Plugin の一覧をテキストファイルに出力できます。

1. **📦 一覧タブ**（またはプラグインタブ）で読み込みを完了させる
2. 進捗バー行の右にある **「📤 リスト出力」** をクリック
3. 保存先と形式（.txt / .csv）を選択

### プラグイン検索

プラグイン名を直接指定してModrinthからダウンロードできます。

1. **🔌 プラグインタブ → 🔎 プラグイン検索 → ⚙ 設定** を開く
2. プラグイン名を入力（またはファイルから読み込む）
3. 保存先フォルダ・Plugin Loaderを選択
4. **「📋 リストに追加」** → **「⬇ 選択をDL」**

> ⚠ 「リストに追加」を押すたびに以前のリストはクリアされます。連続して使用する場合は1回ごとに実行してください。
...
- **🔍 Mod検索タブ**（Mod名 / スラッグIDを直接指定してダウンロード、.txt/.csv/.json 読込対応、ResourcePack / Shader にも対応）
- プラグインの一括アップデート（Modrinth）
- **🔎 プラグイン検索タブ**（プラグイン名指定でModrinthからダウンロード、個別バージョン指定対応）
- **Plugin Loader選択**（Spigot / Paper / Purpur / Velocity等を個別指定、混在防止）
- **プラグイン検査タブ**（JARがPlugin Loader用・プロキシ用・混在のどれかをModrinthで確認）
- **リスト出力**（Mod / ResourcePack / Shader / Plugin の一覧を .txt / .csv で出力）
...
### v1.5.0
- **リスト出力のファイル名形式を統一**（`MCバージョン_種別_yyyy-mm-dd_hh-mm-ss.txt`。プラグインはMCバージョンを含めない）
- **Mod検索・プラグイン検索・プラグイン一覧タブに「📤 リスト出力」を追加**（従来は一括アップデートの一覧タブのみ対応）
- **並列処理に対応**（`ThreadPoolExecutor` による同時ダウンロード。全体設定タブで同時実行数を1〜10の範囲で設定可能。ネットワークI/O待ちが大半のため、並列化しても1件あたりのAPIリクエスト回数は変わらない）
- **APIリクエストの重複排除・キャッシュ化**（同一Mod・同一バージョンへの問い合わせが複数箇所から発生した場合に、実際のリクエストを1回に削減）
- **Modrinthのバルクエンドポイント（`/version_files/update`）を利用した一括バージョン解決**（Mod一括アップデート実行時、既存ファイルのハッシュをまとめて1回のリクエストで問い合わせることで、従来Modごとに2〜3回かかっていたリクエストをほぼ1回に削減）
- Mod検索タブのレイアウト調整（MCバージョンとMod Loaderを同じ行に配置し、ウィンドウの縦幅を圧迫しないよう改善）

### v1.4.0
- **MOD親タブを追加**（設定 / 一覧 / ログ / Mod検索 を子タブとして統合）
- **🔍 Mod検索タブ追加**（Mod名またはスラッグIDを直接指定してダウンロード）
  - .txt / .csv / .json ファイルからMod名を一括読み込み
  - ResourcePack / Shader の検索にも対応
  - 個別バージョン指定・前提Mod自動DL対応
- **🔎 プラグイン検索タブ追加**（plugin_updater に統合）
  - プラグイン名指定でModrinthから直接ダウンロード
  - 個別バージョン指定・サイドパネル付き
- **リスト出力機能追加**（Mod / ResourcePack / Shader / Plugin を .txt / .csv で出力）
- コード軽量化（共通ヘルパー `_t()` / `_tw()` / `_make_log_box()` 導入）
