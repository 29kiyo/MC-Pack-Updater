...
def _apply_theme_globals(theme):
    ...
    t = THEMES[theme]
    BG,BG2,BG3,FG,ACC,GRN,RED,YEL = t["BG"],t["BG2"],t["BG3"],t["FG"],t["ACC"],t["GRN"],t["RED"],t["YEL"]
...
def http_get(url, headers=None):
    ...
    req.add_header("User-Agent","MC-Plugin-Updater/2.0 (github.com/29kiyo/mod-updater)")
    with urllib.request.urlopen(req, timeout=15) as r:
        return json.loads(r.read().decode())

def download_file(url, dest, progress_cb=None):
    req = urllib.request.Request(url)
    req.add_header("User-Agent","MC-Plugin-Updater/2.0")
...
def mr_search_plugin(name, mc_ver):
    try:
        ...
        if mc_ver: facets.append([f"game_versions:{mc_ver}"])
        params = urllib.parse.urlencode({
            "query":name, "limit":5,
...
def mr_get_plugin_versions(pid, mc_ver):
    try:
        p = {}
        if mc_ver: p["game_versions"] = json.dumps([mc_ver])
        p["loaders"] = json.dumps(["paper","spigot","bukkit","purpur","folia","waterfall","velocity"])
...
# ── 統合検索 ──────────────────────────────────────────────────
def find_plugin(name, mc_ver, mode, log_cb, ver_override_id=None):
    ...
    def _try_mr():
        ...
        try:
            ...
            if ver_override_id:
                v_data = http_get(f"{MODRINTH_API}/version/{ver_override_id}")
                vs = [v_data]
            else:
                vs = mr_get_plugin_versions(pid, mc_ver)
    ...
    def _try_sp():
        ...
        try:
            ...
            if res.get("external"):
                log_cb("  Spiget: 外部ホスト（直接DL不可）","warn"); return
            rid = res["id"]
            if ver_override_id:
                versions = spiget_get_versions(rid, size=50)
                matched  = [v for v in versions if ver_override_id in v.get("name","")]
                ver_name = matched[0].get("name","") if matched else ""
            else:
            ...
            source   = "Spiget"
            res_info = {"type":"spiget","id":rid,"resource":res}
...
class PluginUpdaterApp(ttk.Frame):
    def __init__(self, parent, theme="light", icon_path=None, parent_app=None, **kw):
        ...
        self.plugin_list   = []
        self._ver_overrides= {}   # filename -> version_id
    ...
    def _build_list(self, p):
        ...
        sf = ttk.Frame(side_canvas)
        sw = side_canvas.create_window((0,0), window=sf, anchor="nw")
        sf.bind("<Configure>",      lambda e: side_canvas.configure(scrollregion=side_canvas.bbox("all")))
        side_canvas.bind("<Configure>", lambda e: side_canvas.itemconfig(sw, width=e.width))
        ...
        ttk.Button(sf, text="🔄 バージョン取得",
                    command=self._fetch_versions_side).pack(padx=8, pady=(0,8), fill="x")
    ...
    def _update_mode_ui(self):
        ...
        desc = {DL_BOTH_MR:"Modrinthで検索 → なければSpigetも",
    ...
    def _on_select(self, e=None):
        ...
        self._ver_var.set("最新" if not override else override)
        self._ver_status_side.config(text="← バージョン取得ボタンで一覧取得")
        self._versions_cache = []
    ...
    def _fetch_versions_side(self):
        ...
        def _fetch():
            ...
            # Modrinthから試す
            try:
                pid = mr_search_plugin(item.get("name",""), mc_ver)
                if pid:
                    vs = mr_get_plugin_versions(pid, mc_ver)
                    for v in vs:
                        results.append({"label":f"[MR] {v.get('version_number','?')}",
                                         "id":v["id"], "source":"modrinth"})
            except Exception: pass
            # Spigetから試す
            try:
                if mode in (DL_BOTH_MR, DL_BOTH_SP, DL_SP):
                    res = spiget_search(item.get("name",""))
                    if res and not res.get("external"):
                        versions = spiget_get_versions(res["id"])
                        for v in versions:
                            results.append({"label":f"[SP] {v.get('name','?')}",
                                             "id":v.get("name",""), "source":"spiget"})
            except Exception: pass
    ...
    def _update_ver_combo(self, results):
        ...
        self._ver_var.set("最新" if not cur else cur)
    ...
    def _on_ver_select(self, e=None):
        ...
        if label == "最新":
            ...
            ver_id = next((v["id"] for v in self._versions_cache if v["label"]==label), None)
    ...
    def _start_update(self):
        ...
        threading.Thread(target=self._worker,
                          args=(selected, self.target_version.get(), self.dl_mode.get()),
                          daemon=True).start()

    # ── ワーカー ──────────────────────────────────────────────
    def _worker(self, plugins, mc_ver, mode):
        ...
        for i, plugin in enumerate(plugins):
            ...
            dl_url, dl_fname, source, res_info = find_plugin(
                name, mc_ver, mode, _log, ver_override_id=override)
                ...
                if self._do_download(dl_url, dest, name, source,
                    ...
                    if auto_deps:
                        ...
                        for dep_name in dep_names:
                            ...
                            du, df, src, _ = find_plugin(dep_name, mc_ver, mode, _log)
                            if du and df:
...
class StandaloneApp(tk.Tk):
    ...
    def _apply_style(self):
        t = THEMES[self._theme]
        BG_,BG2_,BG3_ = t["BG"],t["BG2"],t["BG3"]
        FG_,ACC_ = t["FG"],t["ACC"]
        s = ttk.Style(self); s.theme_use("clam")
        s.configure("TFrame",            background=BG_)
        s.configure("TLabel",            background=BG_, foreground=FG_, font=("Yu Gothic UI",10))
        s.configure("Hdr.TLabel",        background=BG_, foreground=ACC_, font=("Yu Gothic UI",13,"bold"))
        s.configure("Sub.TLabel",        background=BG_, foreground=FG_, font=("Yu Gothic UI",9))
        s.configure("TButton",           background=ACC_, foreground=t["BTN_FG"],
        ...
        s.configure("TEntry",            fieldbackground=BG2_, foreground=FG_, insertcolor=FG_, relief="flat", padding=4)
        s.configure("TCheckbutton",      background=BG_, foreground=FG_, font=("Yu Gothic UI",10))
        s.map("TCheckbutton",            background=[("active",BG_)])
        s.configure("TCombobox",         fieldbackground=BG2_, foreground=FG_,
                     selectbackground=BG2_, selectforeground=FG_, padding=4)
        s.map("TCombobox",
              fieldbackground=[("readonly",BG2_),("disabled",BG3_)],
              foreground=[("readonly",FG_),("disabled","#6c7086")],
              selectbackground=[("readonly",BG2_)], selectforeground=[("readonly",FG_)])
        s.configure("Treeview",          background=BG2_, foreground=FG_, fieldbackground=BG2_,
                     rowheight=26, font=("Yu Gothic UI",9))
        s.configure("Treeview.Heading",  background=BG_, foreground=ACC_,
                     font=("Yu Gothic UI",9,"bold"), relief="flat")
        s.map("Treeview",                background=[("selected",t["TREE_SEL"])],
                                         foreground=[("selected",t["SEL_FG"])])
        s.map("Treeview.Heading",        background=[("active",BG2_),("pressed",BG2_)],
                                         foreground=[("active",ACC_),("pressed",ACC_)],
                                         relief=[("active","flat"),("pressed","flat")])
        s.configure("TProgressbar",      troughcolor=BG2_, background=ACC_, thickness=8)
        s.configure("TNotebook",         background=BG_, tabmargins=0)
        s.configure("TNotebook.Tab",     background=BG2_, foreground=FG_, padding=[14,7], font=("Yu Gothic UI",10))
        s.map("TNotebook.Tab",           background=[("selected",BG_)], foreground=[("selected",ACC_)])
        s.configure("TLabelframe",       background=BG_, relief="solid", borderwidth=1, bordercolor=BG3_)
        s.configure("TLabelframe.Label", background=BG_, foreground=ACC_, font=("Yu Gothic UI",10,"bold"))
        s.configure("TSeparator",        background=BG3_)
        self.configure(bg=BG_)
